...
if ( is_readable( $npa_puc ) ) {
	...
	if ( class_exists( '\YahnisElsts\PluginUpdateChecker\v5\PucFactory' ) ) {
		...
		$npa_update_checker->setBranch( NPA_Plugin::RELEASE_BRANCH );

		/*
		 * Hand the instance to the plugin class so the Tests tab can assert the
		 * update path is actually wired. Without this the whole block can fail
		 * silently — an empty lib/ makes is_readable() false and auto-updates
		 * just never happen, with nothing anywhere reporting it.
		 */
		NPA_Plugin::$update_checker = $npa_update_checker;
	}
}
unset( $npa_puc );
